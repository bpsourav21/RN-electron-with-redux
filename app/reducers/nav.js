...
export default function reducer(state = initialNavState, action) {
  ...
  switch (action.type) {
    ...
    case "RESET_TO_HOME":
      ...
      break;
    // ------------------------------------------------------------------
    case "RESET_TO_REPORTS":
    nextState = AppNavigator.router.getStateForAction(NavigationActions.navigate({ routeName: "Reports" }), state)
    return nextState
    break; 
    // ------------------------------------------------------------------
    case "RESET_TO_REPORTS_LOCK":
    nextState = AppNavigator.router.getStateForAction(NavigationActions.reset({
      index: 0,
      actions: [
        NavigationActions.navigate({ routeName: 'ReportsPass' })
      ]
    }), state);
    return nextState
    break;      
